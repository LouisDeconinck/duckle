...
More examples live in [`samples/`](samples) - drop the pipeline files into a workspace and open them. Runnable example workspaces (committed data, exercised in CI) live in [`examples/`](examples) - `starter-workspace` covers the basics and `flow-control` covers the `ctl.*` components.
